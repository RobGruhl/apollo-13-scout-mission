...
- **Reading the counts**: `./scripts/ping-census.sh` pulls all of the above from Cloudflare (needs `CLOUDFLARE_READ_TOKEN` in the gitignored `.env`) and prints scouts / finishes-by-game / cards-by-tier. Free-plan path detail goes back 8 days but each query spans ≤24 h, so a running total means walking day-sized windows and summing. Refresh `docs/telemetry-dashboard.html` (a manual snapshot — it cannot refresh itself, the API token can't ship to the browser) at least weekly so jamboree days don't fall off the 8-day window. **The live auto-refreshing dashboard is a separate repo**: `~/Projects/apollo-telemetry` → https://robgruhl.github.io/apollo-telemetry/ (GitHub Actions cron + a laptop launchd `workflow_dispatch` every 30 min; GitHub's cron alone silently drops ~60% of fires)
